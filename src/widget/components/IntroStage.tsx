...
export function IntroStage({ onClose, onMinimize, onExpand, isExpanded, onBack }: IntroStageProps) {
  ...
  // As the chat channel's case-type step (onBack set), skip the legacy hero video
  // and show the compact header + a speaking-but-muted thumbnail beside the
  // greeting (the home menu already played the full video). Home is the cinematic
  // view now, so the no-onBack hero path below is effectively unused.
  ...
  return (
    <div className="fa-view-in flex h-full w-full flex-col bg-white">
      ...
      {/* Scrollable content: greeting + a speaking-but-muted thumbnail, then options. */}
      <div className="flex-1 overflow-y-auto px-5 pb-2">
        <div className="flex items-start gap-3">
          ...
          {/* The attorney thumbnail (speaking, muted) beside the greeting — unless
              the legacy hero video is showing above. */}
          {!hasVideo && <ViewVideoThumb view="chat_intro" className="h-[140px] w-[108px]" />}
        </div>
        <div className="mt-4">

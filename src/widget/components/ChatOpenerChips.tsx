      ...
      {options.map((opt) => {
        ...
        return (
          <button
            ...
            type="button"
            onClick={() => pick(opt)}
            className="inline-flex items-center gap-2 rounded-pill border border-famaash-stroke bg-white px-3.5 py-2 text-[13px] font-medium text-[#1A1A1A] transition-colors hover:bg-[#F5F8FB]"
          >
            {icon && <span className="flex shrink-0 items-center text-[color:var(--practice-accent)]">{icon}</span>}
            {opt}
          </button>
        );
      })}
